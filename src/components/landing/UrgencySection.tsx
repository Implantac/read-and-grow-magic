...
import { Timer, MessageCircle, CheckCircle2, Shield, Headphones, Gift, Sparkles } from 'lucide-react';

interface Props { onWhatsApp: () => void }

const guarantees = [
  { icon: Gift, text: '14 dias grátis para testar', desc: 'Sem compromisso' },
  { icon: Headphones, text: 'Implantação assistida inclusa', desc: 'Suporte dedicado' },
        ...
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 mb-6">
              ...
              <Badge variant="secondary" className="text-primary font-semibold text-xs border border-primary/20 px-4 py-1">Vagas limitadas este mês</Badge>
            </div>
          ...
          <div className="bg-card border border-border/50 rounded-2xl p-6 sm:p-8 shadow-lg mb-10 relative overflow-hidden">
            ...
            <p className="text-center text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] mb-6">O que está incluso na implantação</p>
            <div className="grid sm:grid-cols-2 gap-3">
              {guarantees.map((g) => (
                <div key={g.text} className="flex items-center gap-3 p-4 rounded-xl bg-muted/30 border border-border/30 hover:border-primary/20 transition-colors duration-200">
                  ...
                  <div>
                    <span className="text-sm font-bold block leading-tight">{g.text}</span>
