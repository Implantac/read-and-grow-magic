...
export function useRFIDReaders() {
  ...
  const update = async (id: string, updates: Partial<RFIDReader>) => {
    ...
    const { error } = await supabase.from('rfid_readers').update(mapped as TablesUpdate<'rfid_readers'>).eq('id', id);
...
export function useRFIDTags() {
  ...
  const update = async (id: string, updates: TagUpdate) => {
    const { error } = await supabase.from('rfid_tags').update({ ...updates, updated_at: new Date().toISOString() } as TablesUpdate<'rfid_tags'>).eq('id', id);
