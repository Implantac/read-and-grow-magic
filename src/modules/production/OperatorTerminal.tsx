        ...
        {!currentEntry && operatorName && activeOrders.length > 0 && (
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="p-4">
              ...
              {(() => {
                // Find most urgent OP
                const urgentOP = activeOrders.find(o => o.priority === 'urgent' || (o.due_date && new Date(o.due_date) < new Date()));
                const nearDeadline = activeOrders.find(o => o.due_date && differenceInMinutes(new Date(o.due_date), new Date()) < 24 * 60 && !['completed', 'cancelled'].includes(o.status));
                ...
                return (
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isLate ? 'Esta OP está atrasada!' : target.priority === 'urgent' ? 'Prioridade urgente definida pelo PCP' : 'Prazo mais próximo na fila'}
                      {' | '}{target.produced_quantity}/{target.quantity} produzido
                    </p>
                    <Button size="sm" variant="outline" className="mt-2" onClick={() => setSelectedOrderId(target.id)}>
                      Selecionar esta OP
                    </Button>
                  </div>
                );
