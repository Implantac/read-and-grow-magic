...
import { useCreateOrder, useDeleteOrder, useOrders, useUpdateOrderFields, useUpdateOrderStatus, } from '@/hooks/commercial/useOrders';
import { useClients } from '@/hooks/commercial/useClients';
import { validateOrder } from '@/hooks/commercial/useCommercialRules';
...
export default function OrdersPage() {
    ...
    const orderValidations = useMemo(() => {
        if (!formClient.id || formItems.length === 0)
            return [];
    ...
    const handleApproval = (orderId, field, value) => {
    ...
    return (<PageContainer>
      <PageHeader title="Pedidos de Venda" description="Gerencie e acompanhe o ciclo de vida dos seus pedidos">
        <ExportButton data={filteredOrders} columns={[
            { key: 'number', label: 'Pedido' }, { key: 'client_name', label: 'Cliente' },
            { key: 'date', label: 'Data', format: (v) => formatDate(v) },
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <ToggleGroup type="single" value={viewMode} onValueChange={(v) => v && setViewMode(v)} className="shrink-0">
      ...
      {filteredOrders.length === 0 ? (<EmptyState title="Nenhum pedido encontrado" description="Tente ajustar os filtros ou crie um novo pedido de venda para começar." action={{ label: "Novo Pedido", onClick: () => { resetForm(); setIsFormOpen(true); } }}/>) : viewMode === 'table' ? (<OrdersTable orders={filteredOrders} selectedOrder={selectedOrder} isDeletePending={deleteOrder.isPending} isAdvancePending={updateStatus.isPending} onView={(order) => { setSelectedOrder(order); setIsViewOpen(true); }} onAdvance={handleAdvanceStatus} onAskDelete={(order) => { setSelectedOrder(order); setIsDeleteConfirmOpen(true); }} onAskCancel={(order) => { setSelectedOrder(order); setIsCancelOpen(true); }}/>) : (<SalesKanbanBoard orders={filteredOrders} isAdvancePending={updateStatus.isPending} onView={(order) => { setSelectedOrder(order); setIsViewOpen(true); }} onAdvance={handleAdvanceStatus} onAskCancel={(order) => { setSelectedOrder(order); setIsCancelOpen(true); }}/>)}
      ...
      <ViewOrderDialog open={isViewOpen} onOpenChange={setIsViewOpen} order={selectedOrder} onApprove={handleApproval} onAdvance={handleAdvanceStatus} onAskCancel={() => setIsCancelOpen(true)}/>

      <DeleteOrderDialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen} orderNumber={selectedOrder?.number} isPending={deleteOrder.isPending} onCancel={() => setSelectedOrder(null)} onConfirm={confirmDelete}/>

      <CancelOrderDialog open={isCancelOpen} onOpenChange={setIsCancelOpen} orderNumber={selectedOrder?.number} onConfirm={handleCancel}/>
    </PageContainer>);
}
