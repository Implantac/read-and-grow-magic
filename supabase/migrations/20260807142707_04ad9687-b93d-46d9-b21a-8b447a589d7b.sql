...
CREATE TABLE IF NOT EXISTS public.replenishment_policies (
    ...
    replenishment_source_id UUID REFERENCES public.branches(id),
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'pos_terminals_tenant_isolation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'pos_sessions_tenant_isolation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'stock_transfer_orders_tenant_isolation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'stock_transfer_items_tenant_isolation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'replenishment_policies_tenant_isolation') THEN
