CREATE OR REPLACE FUNCTION public.aps_schedule_multi(_horizon_days integer DEFAULT 14)
 RETURNS TABLE(order_id uuid, order_number text, product_name text, quantity numeric, due_date timestamp with time zone, priority text, machine_id uuid, machine_name text, machine_sector text, operator text, duration_minutes integer, planned_start timestamp with time zone, planned_end timestamp with time zone, is_late boolean, conflicts text[], sequence_no integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  DELETE FROM _aps_clock WHERE machine_id IS NOT NULL OR machine_id IS NULL;
